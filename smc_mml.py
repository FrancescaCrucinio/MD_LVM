...
def smc_mml_multimodal(N, Niter, data):
    ...
    theta[0, :] = np.random.uniform(-50, 50, size = N)
    ...
    for n in range(1, Niter):
        ...
        for i in range(4):
            incremental_w = incremental_w - 0.525*np.log(0.05+(data[i] - theta[n-1, :])**2)
        logW = logW + incremental_w
        W = rs.exp_and_normalise(logW)
        if(rs.essl(logW)<N/2):
            ancestors = rs.resampling('stratified', W)
            theta[n, :] = theta[n-1, ancestors]
            logW = np.zeros(N)
        z = np.zeros((N, 4))
        for k in range(N):
            for i in range(4):
                z[k, i] = np.random.gamma(shape = 0.525, scale =1/(0.025+(data[i]-theta[n, k])**2/2), size = 1)
            if(n>1):
                sigma_old = sigma[k]
                mu_old = mu[k]
            ...
            theta[n, k] = np.random.normal(loc = mu[k], scale = np.sqrt(sigma[k]), size = 1)
        theta_estimate[n] = np.sum(theta[n, :]*W)
    return theta_estimate, z, W, theta